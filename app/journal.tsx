...
function JournalSession({initial, switching, switchDataset}: {initial: SessionData; switching: boolean; switchDataset: (demo: boolean) => Promise<void>}) {
    ...
    const [layout, setLayout] = useState(() => preference('shizuku-layout-experiment-v3', 'standard', layouts)), [designOpen, setDesignOpen] = useState(false), [ui, setUi] = useState(() => preference('shizuku-ui', 'shizuku', interfaces)), [composerOpen, setComposerOpen] = useState(false), [recent, setRecent] = useState<string[]>(initial.tags.recent);
    ...
    const composer = <section className={`checkin-card ${more ? 'expanded' : ''}`}><div className="section-heading"><h2>いまの体調は？</h2><span className="tap-hint">選んで、送信</span></div><fieldset disabled={busy || !!failed}><MoodButtons selected={draft.score} onPick={score => setDraft({ ...draft, score: draft.score === score ? null : score })}/><div className="composer"><textarea aria-label="ひとことメモ" value={draft.note} maxLength={4000} onChange={e => setDraft({ ...draft, note: e.target.value })} placeholder="ひとこと残す？ 書かなくても大丈夫。" rows={1}/><TagPicker all={displayedTags} value={draft.tags} catalog={catalog} usage={usage} recent={recent} expanded={more} quantities={draft.quantities} onQuantities={quantities => setDraft({ ...draft, quantities })} onChange={tags => setDraft({ ...draft, tags, quantities: Object.fromEntries(tags.map(t => [t, draft.quantities[t] ?? 1])) })} onAdd={() => { setNewTagGroup('未分類'); setTagOpen(true); }}/>{more && <label className="backdate-field"><span>記録する日時</span><small>空欄なら、送信した時刻</small><input className="text-input" type="datetime-local" aria-label="投稿の日時" max={localInput(new Date().toISOString())} value={backdate} onInput={e => setBackdate(e.currentTarget.value)} onChange={e => setBackdate(e.target.value)}/></label>}<div className="composer-footer"><button type="button" className="details-toggle" onClick={() => setMore(!more)} aria-expanded={more}>{more ? <><ChevronUp size={16}/>タグ・日時を閉じる</> : <><SlidersHorizontal size={16}/>タグ・日時</>}</button><button type="button" className="send-button" disabled={busy || (!draft.note.trim() && !draft.tags.length && draft.score === null)} onClick={submit}>{busy ? <LoaderCircle size={17} className="spin"/> : <Send size={17}/>}送信</button></div></div></fieldset>{failed && <div className="error-banner" role="alert">まだ保存を確認できていません。<button disabled={busy} onClick={submit}>同じ記録を再送</button></div>}</section>;
