...
import { RecordLayoutView } from '@/components/journal/record-layout';
import { layouts, getLayout, type RecordLayout } from '@/lib/layouts';
...
function preference(key: string, fallback: string, options: readonly {id: string}[]) {
...
function JournalSession({initial, switching, switchDataset}: {initial: SessionData; switching: boolean; switchDataset: (demo: boolean) => Promise<void>}) {
    ...
    const [layout, setLayout] = useState(() => preference('shizuku-layout-experiment-v3', 'standard', layouts)), [designOpen, setDesignOpen] = useState(false), [ui, setUi] = useState(() => preference('shizuku-ui', 'shizuku', interfaces)), [composerOpen, setComposerOpen] = useState(false), [reading, setReading] = useState(false);
    ...
    function chooseLayout(value: string) { if (!value) return; setLayout(value); setComposerOpen(false); setReading(false); try { localStorage.setItem('shizuku-layout-experiment-v3', value); } catch { } }
    ...
    const experience = getLayout(layout);
    const dayDate = filters.date || (demo ? overview.days.at(-1)?.date || today : today);
    const dayStat = overview.days.find(d => d.date === dayDate);
    const recordFilters = experience.history === 'day' ? { ...filters, date: dayDate } : filters;
    function openComposer() {
        // 配置を替えても入力途中の内容・日時はそのまま。空の下書きだけ表示日に合わせる。
        if (experience.history === 'day' && !draft.note && !draft.tags.length && draft.score === null) {
            setBackdate(dayDate === today ? '' : `${dayDate}T${localInput(new Date().toISOString()).slice(11)}`);
        }
    ...
    const composer = <section className={`checkin-card ${more ? 'expanded' : ''}`}>
      ...
      <fieldset disabled={busy || !!failed}>
        ...
        <div className="composer">
          ...
          <TagPicker all={displayedTags} value={draft.tags} catalog={catalog} usage={usage} quickLimit={['dock', 'focus'].includes(layout) ? 3 : 6} expanded={more} onToggle={() => setMore(v => !v)}
    ...
    const layoutOptions = (items: readonly RecordLayout[], label: string) => <ToggleGroup type="single" value={layout} onValueChange={chooseLayout} className="layout-options" aria-label={label}>{items.map(l => <ToggleGroupItem key={l.id} value={l.id} className="layout-option" aria-label={`${l.name}の配置`}><span><strong>{l.name}</strong><small>{l.hint}</small></span>{layout === l.id && <Check size={18}/>}</ToggleGroupItem>)}</ToggleGroup>;
    return <div className="app-shell" data-layout={layout} data-ui={ui} data-record-active={tab === 'record'}><Toaster theme={skin === 'paper' ? 'light' : 'dark'} position="top-center" richColors/><header className="app-header"><Link href={demo ? "/?demo=1" : "/"} className="brand"><span className="brand-icon"><Droplet size={24}/></span><span>しずく<small>体調の記録</small></span></Link><div className="header-actions"><span className="private-label"><LockKeyhole size={14}/><span>{demo ? "試用データ" : "自分だけの記録"}</span></span><button className="icon-button" aria-label="デザイン実験" title="デザイン実験" onClick={() => setDesignOpen(true)}><FlaskConical size={20}/></button><button className="icon-button skin-trigger" aria-label="見た目を変える" onClick={() => setSkinOpen(true)}><Palette size={21}/></button></div></header>
 ...
 <TabsContent value="record" forceMount className="tab-panel"><RecordLayoutView
    layout={experience} composer={composer} date={dayDate} score={draft.score}
    dayCount={dayStat?.count ?? 0} dayScore={dayStat?.score ?? null}
    reading={reading} onReading={setReading} onDate={date => setFilters({ ...filters, date })} onCompose={openComposer}
    history={experience.history === 'axis' ? <TimeAxis revision={revision} active={tab === 'record'} selectedDate={filters.date || (demo ? overview.days.at(-1)?.date || today : today)} onDateChange={date => setFilters({...filters, date})} onEdit={setEditing}/> : <Timeline initialPage={initial.entries} visible={tab === 'record' && (layout !== 'focus' || reading)} revision={revision} tags={allTags} filters={recordFilters} dateLocked={experience.history === 'day'} setFilters={setFilters} onEdit={e => setEditing({ ...e, tags: [...e.tags] })}/>}
    summary={<aside className="summary-column"><section className="summary-card"><div className="section-heading"><h2>今日、その時々の体調</h2><Activity size={18}/></div><div className="daily-number">{fmt(todayStat?.score)}<span>/ 5</span></div><p className="metadata">{todayStat?.scoreCount ? `${todayStat.scoreCount}回の体調の平均` : '体調の投稿はまだありません'}</p><div className="summary-divider"/><div className="row"><span>一日の総括</span><strong>{todaySummary ? `${todaySummary.score} / 5` : '未記録'}</strong></div><button className="subtle-link" onClick={() => setTab('calendar')}>{todaySummary ? 'カレンダーを見る' : '今日を振り返る'}<ArrowUpRight size={16}/></button></section><section className="week-card"><div className="section-heading"><h2>最近7日の総括</h2></div><div className="week-strip">{Array.from({ length: 7 }, (_, i) => shiftDay(today, i - 6)).map(d => { const s = overview.summaries.find(s => s.date === d); return <div key={d}><span>{Number(d.slice(8))}</span><strong className={s ? 'mood-' + s.score : ''}>{s?.score ?? '—'}</strong></div>; })}</div><p className="form-hint">空白は、まだ振り返っていない日。</p></section></aside>}/></TabsContent>
 <TabsContent value="calendar" forceMount className="tab-panel"><CalendarView overview={overview} disabled={loading || !!error} onSaved={refresh} onViewTrends={() => setTab('trends')} onViewEntries={date => { setFilters({ q: '', tag: '', date }); setReading(true); setTab('record'); }}/></TabsContent>
 <TabsContent value="trends" forceMount className="tab-panel"><Trends overview={overview} active={tab === 'trends'} demo={demo} onViewEntries={date => { setFilters({ q: '', tag: '', date }); setReading(true); setTab('record'); }}/></TabsContent><TabsContent value="tags" forceMount className="tab-panel"><TagManager catalog={catalog} usage={usage} busy={tagBusy || !tagReady} onSave={saveCatalog}/></TabsContent></Tabs>
 {tab === 'record' && ['thumb', 'chronicle'].includes(layout) && <button className="floating-compose" onClick={openComposer}><Plus size={22}/>記録する</button>}
 <Dialog open={composerOpen} onOpenChange={v => !busy && setComposerOpen(v)}><DialogContent className="journal-dialog composer-sheet"><DialogTitle>{backdate ? `${dateText(backdate.slice(0, 10))}の記録` : 'その時の記録'}</DialogTitle><DialogDescription>体調だけでも、ひとことだけでも。</DialogDescription>{experience.composer === 'sheet' && composer}</DialogContent></Dialog>
