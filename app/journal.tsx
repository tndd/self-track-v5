...
const layouts = [{ id: 'standard', name: '標準', hint: '入力の下に履歴。広い画面では右に集計。' }, { id: 'notebook', name: 'ノート', hint: '一列に集中。カードを減らした一覧。' }, { id: 'studio', name: 'ワークスペース', hint: '広い画面は左に入力、右に履歴。履歴を広く表示。' }];
type Draft = {
    note: string;
    tags: string[];
    score: number | null;
...
export default function Journal() {
    ...
    const [layout, setLayout] = useState('standard'), [designOpen, setDesignOpen] = useState(false);
    ...
    const refresh = useCallback(async () => { const gen = ++overviewGeneration.current; try {
        const [data, tagData] = await Promise.all([request<Overview>('/api/overview'), request<{catalog: Catalog; usage: Record<string, number>}>('/api/tags')]);
    ...
    useEffect(() => { try {
        const value = localStorage.getItem('shizuku-layout-experiment-v2');
    ...
    function chooseLayout(value: string) { if (!value) return; setLayout(value); try { localStorage.setItem('shizuku-layout-experiment-v2', value); } catch { } }
 ...
 <Dialog open={designOpen} onOpenChange={setDesignOpen}><DialogContent className="journal-dialog design-dialog"><DialogTitle>デザイン実験</DialogTitle><DialogDescription>配置を比べるための試験機能です。記録と入力途中の内容はそのまま、いつでも標準に戻せます。</DialogDescription><ToggleGroup type="single" value={layout} onValueChange={chooseLayout} className="layout-options" aria-label="配置の候補">{layouts.map(l => <ToggleGroupItem key={l.id} value={l.id} className="layout-option" aria-label={`${l.name}の配置`}><span className={`layout-mini layout-mini-${l.id}`} aria-hidden="true"><i/><b/><em/></span><span><strong>{l.name}</strong><small>{l.hint}</small></span>{layout === l.id && <Check size={18}/>}</ToggleGroupItem>)}</ToggleGroup><div className="row"><button className="text-button" onClick={() => chooseLayout('standard')}>標準に戻す</button><button className="primary-button" onClick={() => setDesignOpen(false)}>この配置で試す</button></div></DialogContent></Dialog>
