...
import { Activity, ArrowUpRight, CalendarDays, Check, ChevronUp, Download, Droplet, LoaderCircle, LockKeyhole, FlaskConical, Palette, Plus, Send, SlidersHorizontal, Trash2, TrendingUp, Tags } from 'lucide-react';
...
import { defaultCatalog, catalogSchema, type Catalog } from '@/lib/tags';
...
const interfaces = [{ id: 'shizuku', name: 'しずく標準', hint: '夜の青と、やわらかな丸み。' }, { id: 'apple', name: 'Apple風', hint: '大きな見出しと、まとまりのある面。' }, { id: 'google', name: 'Google風', hint: '丸い操作部と、はっきりした強弱。' }, { id: 'openai', name: 'OpenAI風', hint: '余白と文字を主役にした、シンプルな画面。' }];
...
export default function Journal() {
    const [overview, setOverview] = useState<Overview>(emptyOverview), [loading, setLoading] = useState(true), [error, setError] = useState(''), [busy, setBusy] = useState(false), [tab, setTab] = useState('record'), [draft, setDraft] = useState<Draft>(blank), [more, setMore] = useState(false), [backdate, setBackdate] = useState(''), [tagOpen, setTagOpen] = useState(false), [newTag, setNewTag] = useState(''), [editing, setEditing] = useState<Entry | null>(null), [deleteId, setDeleteId] = useState<string | null>(null), [filters, setFilters] = useState<Filters>({ q: '', tag: '', date: '' }), [revision, setRevision] = useState(0), [skin, setSkin] = useState('night'), [skinOpen, setSkinOpen] = useState(false), [exporting, setExporting] = useState(false), [failed, setFailed] = useState<EntryInput | null>(null);
    const [catalog, setCatalog] = useState<Catalog>(defaultCatalog), [usage, setUsage] = useState<Record<string, number>>({}), [tagBusy, setTagBusy] = useState(false), [tagReady, setTagReady] = useState(false), [newTagGroup, setNewTagGroup] = useState('未分類');
    const [layout, setLayout] = useState('standard'), [designOpen, setDesignOpen] = useState(false), [ui, setUi] = useState('shizuku'), [composerOpen, setComposerOpen] = useState(false), [recent, setRecent] = useState<string[]>([]), [bookDate, setBookDate] = useState(todayKey);
    const [demo, setDemo] = useState(false), [demoBusy, setDemoBusy] = useState(false);
    useEffect(() => {
        // URLで選ぶ試用モードは、認証済み利用者の別領域を使う。
        // eslint-disable-next-line react-hooks/set-state-in-effect
        setDemo(new URLSearchParams(window.location.search).get('demo') === '1');
    }, []);
    async function openDemo() { setDemoBusy(true); try { await request('/api/demo', {method:'POST',headers:{'X-Shizuku-Dataset':'demo'}}); window.location.assign(new URL('/?demo=1',window.location.origin).href); } catch(e) {toast.error((e as Error).message); setDemoBusy(false);} }
    ...
    const refresh = useCallback(async () => { const gen = ++overviewGeneration.current; try {
        ...
        if (gen === overviewGeneration.current) {
            setOverview(data);
            setCatalog(old => tagData.catalog.revision >= old.revision ? tagData.catalog : old); setUsage(tagData.usage); setRecent(tagData.recent); setTagReady(true);
            setError('');
        }
    }
    catch (e) {
    ...
    useEffect(() => { try {
        const value = localStorage.getItem('shizuku-layout-experiment-v3');
        if (layouts.some(l => l.id === value)) {
            // ブラウザーに保存した試験設定を、マウント後に同期する。
            // eslint-disable-next-line react-hooks/set-state-in-effect
            setLayout(value!);
        }
    } catch { } }, []);
    useEffect(() => { try {
        const value = localStorage.getItem('shizuku-ui');
        if (interfaces.some(i => i.id === value)) {
            // eslint-disable-next-line react-hooks/set-state-in-effect
            setUi(value!); document.documentElement.dataset.ui = value!;
        }
    } catch { } }, []);
    ...
    const composer = <section className={`checkin-card ${more ? 'expanded' : ''}`}><div className="section-heading"><h2>いまの体調は？</h2><span className="tap-hint">選んで、送信</span></div><fieldset disabled={busy || !!failed}><MoodButtons selected={draft.score} onPick={score => setDraft({ ...draft, score: draft.score === score ? null : score })}/><div className="composer">{more && <div className="composer-expanded-bar"><button type="button" className="details-toggle" onClick={() => setMore(false)} aria-expanded="true"><ChevronUp size={16}/>タグ・日時を閉じる</button></div>}<textarea aria-label="ひとことメモ" value={draft.note} maxLength={4000} onChange={e => setDraft({ ...draft, note: e.target.value })} placeholder="ひとこと残す？ 書かなくても大丈夫。" rows={1}/><TagPicker all={displayedTags} value={draft.tags} catalog={catalog} usage={usage} recent={recent} expanded={more} quantities={draft.quantities} onQuantities={quantities => setDraft({ ...draft, quantities })} onChange={tags => setDraft({ ...draft, tags, quantities: Object.fromEntries(tags.map(t => [t, draft.quantities[t] ?? 1])) })} onAdd={() => { setNewTagGroup('未分類'); setTagOpen(true); }}/>{more && <label className="backdate-field">記録する日時 <small>空欄なら、送信した時刻</small><input className="text-input" type="datetime-local" aria-label="投稿の日時" max={localInput(new Date().toISOString())} value={backdate} onInput={e => setBackdate(e.currentTarget.value)} onChange={e => setBackdate(e.target.value)}/></label>}<div className="composer-footer"><button type="button" className={`details-toggle ${more ? 'details-toggle-bottom' : ''}`} onClick={() => setMore(!more)} aria-label={more ? '下のボタンでタグ・日時を閉じる' : undefined} aria-expanded={more}>{more ? <ChevronUp size={15}/> : <><SlidersHorizontal size={16}/>タグ・日時</>}</button><button type="button" className="send-button" disabled={busy || (!draft.note.trim() && !draft.tags.length && draft.score === null)} onClick={submit}>{busy ? <LoaderCircle size={17} className="spin"/> : <Send size={17}/>}送信</button></div></div></fieldset>{failed && <div className="error-banner" role="alert">まだ保存を確認できていません。<button disabled={busy} onClick={submit}>同じ記録を再送</button></div>}</section>;
 ...
 {demo && <aside className="demo-banner"><div><FlaskConical size={17}/><span><strong>試用モード</strong><small>4か月分の架空の記録。自由に追加・編集できます。</small></span></div><button className="text-button" onClick={() => window.location.assign(new URL('/',window.location.origin).href)}>自分の記録へ</button></aside>}
 ...
 <TabsContent value="record" forceMount className="tab-panel"><div className="page-heading"><div><p className="eyebrow">A MOMENT, KEPT</p><h1>その時の記録</h1></div><span className="heading-note">{dateText(today)}</span></div><div className="record-grid"><div className="record-main">{layout === 'daybook' && <section className="daybook-overview"><div className="book-date"><label htmlFor="book-date">一日のページ</label><input id="book-date" type="date" className="text-input" value={bookDate} max={today} onChange={e => { if (e.target.value && e.target.value <= today) setBookDate(e.target.value); }}/></div><div className="book-week">{Array.from({length: 7}, (_, i) => shiftDay(bookDate, i - 3)).map(d => <button key={d} disabled={d > today} className={d === bookDate ? 'selected' : ''} aria-label={`${d}のページ`} aria-pressed={d === bookDate} onClick={() => setBookDate(d)}><span>{new Intl.DateTimeFormat('ja-JP', {weekday: 'short', timeZone: 'Asia/Tokyo'}).format(new Date(d + 'T12:00:00+09:00'))}</span><strong>{Number(d.slice(8))}</strong></button>)}</div><div className="book-summary"><span>一日の総括 <b>{overview.summaries.find(d => d.date === bookDate)?.score ?? '—'}</b></span><span>投稿 <b>{overview.days.find(d => d.date === bookDate)?.count ?? 0}件</b></span></div>{overview.summaries.find(d => d.date === bookDate)?.note && <p className="book-note">{overview.summaries.find(d => d.date === bookDate)?.note}</p>}<p className="form-hint">入力は現在時刻。「タグ・日時」で過去の日時も指定できます。</p></section>}{!['thumb', 'chronicle'].includes(layout) && composer}
 {layout === 'chronicle' ? <TimeAxis revision={revision} active={tab === 'record'} onEdit={setEditing}/> : <Timeline dateLocked={layout === 'daybook'} revision={revision} tags={allTags} filters={layout === 'daybook' ? { ...filters, date: bookDate } : filters} setFilters={f => { setFilters(f); if (layout === 'daybook' && f.date) setBookDate(f.date); }} onEdit={e => setEditing({ ...e, tags: [...e.tags] })}/>}</div><aside className="summary-column"><section className="summary-card"><div className="section-heading"><h2>今日、その時々の体調</h2><Activity size={18}/></div><div className="daily-number">{fmt(todayStat?.score)}<span>/ 5</span></div><p className="metadata">{todayStat?.scoreCount ? `${todayStat.scoreCount}回の体調の平均` : '体調の投稿はまだありません'}</p><div className="summary-divider"/><div className="row"><span>一日の総括</span><strong>{todaySummary ? `${todaySummary.score} / 5` : '未記録'}</strong></div><button className="subtle-link" onClick={() => setTab('calendar')}>{todaySummary ? 'カレンダーを見る' : '今日を振り返る'}<ArrowUpRight size={16}/></button></section><section className="week-card"><div className="section-heading"><h2>最近7日の総括</h2></div><div className="week-strip">{Array.from({ length: 7 }, (_, i) => shiftDay(today, i - 6)).map(d => { const s = overview.summaries.find(s => s.date === d); return <div key={d}><span>{Number(d.slice(8))}</span><strong className={s ? 'mood-' + s.score : ''}>{s?.score ?? '—'}</strong></div>; })}</div><p className="form-hint">空白は、まだ振り返っていない日。</p></section></aside></div></TabsContent>
 ...
 <TabsContent value="trends" forceMount className="tab-panel"><Trends overview={overview} revision={revision} active={tab === 'trends'}/></TabsContent><TabsContent value="tags" forceMount className="tab-panel"><TagManager catalog={catalog} usage={usage} busy={tagBusy || !tagReady} onSave={saveCatalog}/></TabsContent></Tabs>
