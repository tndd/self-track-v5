...
import { Activity, ArrowUpRight, CalendarDays, Check, Download, Droplet, LoaderCircle, LockKeyhole, FlaskConical, Palette, Plus, Send, SlidersHorizontal, Trash2, TrendingUp } from 'lucide-react';
...
const layouts = [{ id: 'standard', name: '標準', hint: '入力の下に履歴。広い画面では右に集計。' }, { id: 'notebook', name: 'ノート', hint: '一列に集中。カードを減らした一覧。' }, { id: 'studio', name: 'ワークスペース', hint: '広い画面は左に入力、右に履歴。スマホは下にナビ。' }];
type Draft = {
    note: string;
    tags: string[];
    score: number | null;
...
export default function Journal() {
    ...
    useEffect(() => { try {
        const value = localStorage.getItem('shizuku-layout-experiment');
    ...
    function chooseLayout(value: string) { if (!value) return; setLayout(value); try { localStorage.setItem('shizuku-layout-experiment', value); } catch { } }
