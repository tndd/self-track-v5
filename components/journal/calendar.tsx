...
export function CalendarView({ overview, onSaved, onViewEntries, disabled }: {
    ...
    const today = todayKey(), [month, setMonth] = useState(today.slice(0, 7)), [date, setDate] = useState(today), [drafts, setDrafts] = useState<Record<string, {
        score: number | null;
        note: string;
    }>>({}), [busy, setBusy] = useState(false), [removeOpen, setRemoveOpen] = useState(false);
 ...
 <section className="summary-editor"><div className="section-heading"><div><p className="eyebrow">DAILY REFLECTION</p><h2>{dateText(date)}</h2></div><CalendarDays size={21}/></div><p className="summary-status">{saved ? <><Check size={15}/>総括を記録済み</> : 'この日の総括は、まだ空白です'}</p><fieldset disabled={disabled || busy}><legend className="field-label">一日全体の体調</legend><MoodButtons label="一日の総括" selected={draft.score} onPick={score => update({ score })}/><label className="field-label" htmlFor="day-note">一日のコメント <small>任意</small></label><textarea id="day-note" className="text-input" rows={4} maxLength={4000} placeholder="一日を通して、どうだった？" value={draft.note} onChange={e => update({ note: e.target.value })}/><button className="primary-button full-width" disabled={disabled || busy || draft.score === null} onClick={() => void save()}>{busy ? '保存中…' : '総括を保存'}</button></fieldset><p className="form-hint">その時々の体調とは、別の記録です。</p><div className="summary-actions"><button className="text-button" onClick={() => onViewEntries(date)}>この日の投稿を見る（{count}件）</button>{saved && <button className="icon-button danger-button" disabled={busy} aria-label="この日の総括を削除" onClick={() => setRemoveOpen(true)}><Trash2 size={17}/></button>}</div></section></div><AlertDialog open={removeOpen} onOpenChange={v => !busy && setRemoveOpen(v)}><AlertDialogContent><AlertDialogTitle>この日の総括を削除しますか？</AlertDialogTitle><AlertDialogDescription>その時々の投稿は残ります。総括は元に戻せません。</AlertDialogDescription><AlertDialogFooter><AlertDialogCancel disabled={busy}>残す</AlertDialogCancel><AlertDialogAction disabled={busy} onClick={e => { e.preventDefault(); void remove(); }}>総括を削除</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog></>;
}
