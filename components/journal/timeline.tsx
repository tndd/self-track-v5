...
export function Timeline({ revision, tags, filters, setFilters, onEdit }: {
 ...
 {groups.map(d => <section className="history-day" key={d}><div className="day-heading"><h3>{d === todayKey() ? '今日 · ' : ''}{dateText(d)}</h3></div>{entries.filter(e => e.date === d).map(e => <article className={`entry ${!e.note && !e.tags.length ? 'score-only' : ''}`} key={e.id} data-entry-id={e.id}><div className="entry-rail"><span className={`entry-mood mood-${e.score ?? 3}`} role="img" aria-label={e.score !== null ? `体調${e.score} ${MOODS[e.score - 1].label}` : "体調なし・メモ"} title={e.score !== null ? `${e.score} · ${MOODS[e.score - 1].label}` : "メモ"}>{e.score !== null ? <MoodIcon score={e.score}/> : <MessageCircle size={24} aria-hidden="true"/>}</span><time dateTime={e.recordedAt} className="entry-time">{time(e.recordedAt)}</time></div><div className="entry-bubble"><div className="entry-top"><button className="icon-button" aria-label={`${d} ${time(e.recordedAt)}の記録を編集`} onClick={() => onEdit(e)}><MoreHorizontal size={20}/></button></div>{e.note && <p className="entry-note">{e.note}</p>}{e.tags.length > 0 && <div className="entry-tags">{e.tags.map(t => <button key={t} onClick={() => setFilters({ ...filters, tag: t })}>#{t}{(e.quantities?.[t] ?? 1) !== 1 ? ` ×${e.quantities?.[t]}` : ''}</button>)}</div>}</div></article>)}</section>)}
