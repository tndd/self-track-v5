...
import { MoodIcon, dateText, time, request } from './shared';
export type Filters = {
    q: string;
    tag: string;
    date: string;
};
export function Timeline({ revision, tags, filters, setFilters, onEdit, dateLocked = false }: {
    revision: number;
    dateLocked?: boolean;
    tags: string[];
    filters: Filters;
 ...
 {(open || active) && <div className="history-tools"><label className="search-field"><Search size={18}/><input aria-label="言葉で検索" placeholder="メモやタグを検索" value={filters.q} onChange={e => setFilters({ ...filters, q: e.target.value })}/></label><Select value={filters.tag || '__all'} onValueChange={v => setFilters({ ...filters, tag: v === '__all' ? '' : v })}><SelectTrigger aria-label="タグで絞り込む"><SelectValue placeholder="すべてのタグ"/></SelectTrigger><SelectContent><SelectItem value="__all">すべてのタグ</SelectItem>{tags.map(t => <SelectItem key={t} value={t}>#{t}</SelectItem>)}</SelectContent></Select>{!dateLocked && <input className="history-date" type="date" aria-label="履歴の日付" max={todayKey()} value={filters.date} onInput={e => setFilters({ ...filters, date: e.currentTarget.value })} onChange={e => setFilters({ ...filters, date: e.target.value })}/>} {active && <button className="text-button" onClick={() => setFilters({ q: '', tag: '', date: dateLocked ? filters.date : '' })}><X size={14}/>解除</button>}</div>}
 {groups.map(d => <section className="history-day" key={d}><div className="day-heading"><h3>{d === todayKey() ? '今日 · ' : ''}{dateText(d)}</h3></div>{entries.filter(e => e.date === d).map((e, i, dayEntries) => <Fragment key={e.id}>{i > 0 && !entriesConnected(dayEntries[i - 1], e) && <div className="entry-gap">{gapLabel(entryInterval(dayEntries[i - 1], e))}の間隔</div>}<article className={`entry ${!e.note && !e.tags.length ? 'score-only' : ''} ${dayEntries[i + 1] && entriesConnected(e, dayEntries[i + 1]) ? 'connected-next' : ''} ${i > 0 && entriesConnected(dayEntries[i - 1], e) ? 'connected-prev' : ''}`}  data-entry-id={e.id}><div className="entry-rail"><span className={`entry-mood mood-${e.score ?? 3}`} role="img" aria-label={e.score !== null ? `体調${e.score} ${MOODS[e.score - 1].label}` : "体調なし・メモ"} title={e.score !== null ? `${e.score} · ${MOODS[e.score - 1].label}` : "メモ"}>{e.score !== null ? <MoodIcon score={e.score}/> : <MessageCircle size={24} aria-hidden="true"/>}</span><time dateTime={e.recordedAt} className="entry-time">{time(e.recordedAt)}</time></div><div className="entry-bubble"><div className="entry-top"><button className="icon-button" aria-label={`${d} ${time(e.recordedAt)}の記録を編集`} onClick={() => onEdit(e)}><MoreHorizontal size={20}/></button></div>{e.note && <p className="entry-note">{e.note}</p>}{e.tags.length > 0 && <div className="entry-tags">{e.tags.map(t => <button key={t} onClick={() => setFilters({ ...filters, tag: t })}>#{t}{(e.quantities?.[t] ?? 1) !== 1 ? ` ×${e.quantities?.[t]}` : ''}</button>)}</div>}</div></article></Fragment>)}</section>)}
