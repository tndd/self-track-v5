...
export function Timeline({ revision, tags, filters, setFilters, onEdit, dateLocked = false }: {
 ...
 {groups.map(d => <section className="history-day" key={d}><div className="day-heading"><h3>{d === todayKey() ? '今日 · ' : ''}{dateText(d)}</h3></div>{entries.filter(e => e.date === d).map((e, i, dayEntries) => { const score = e.score ?? 3; return <Fragment key={e.id}>{i > 0 && !entriesConnected(dayEntries[i - 1], e) && <div className="entry-gap">{gapLabel(entryInterval(dayEntries[i - 1], e))}の間隔</div>}<article className={`entry ${!e.note && !e.tags.length ? 'score-only' : ''} ${dayEntries[i + 1] && entriesConnected(e, dayEntries[i + 1]) ? 'connected-next' : ''} ${i > 0 && entriesConnected(dayEntries[i - 1], e) ? 'connected-prev' : ''}`}  data-entry-id={e.id}><div className="entry-rail"><span className={`entry-mood mood-${score}`} role="img" aria-label={`体調${score} ${MOODS[score - 1].label}`} title={`${score} · ${MOODS[score - 1].label}`}><MoodIcon score={score}/></span><time dateTime={e.recordedAt} className="entry-time">{time(e.recordedAt)}</time></div><div className="entry-bubble"><div className="entry-top"><button className="icon-button" aria-label={`${d} ${time(e.recordedAt)}の記録を編集`} onClick={() => onEdit(e)}><MoreHorizontal size={20}/></button></div>{e.note && <p className="entry-note">{e.note}</p>}{e.tags.length > 0 && <div className="entry-tags">{e.tags.map(t => <button key={t} onClick={() => setFilters({ ...filters, tag: t })}>#{t}{(e.quantities?.[t] ?? 1) !== 1 ? ` ×${e.quantities?.[t]}` : ''}</button>)}</div>}</div></article></Fragment>; })}</section>)}
