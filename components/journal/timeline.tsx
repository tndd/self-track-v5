...
import { Search, SlidersHorizontal, MoreHorizontal, MessageCircle, LoaderCircle, X, CalendarDays } from 'lucide-react';
...
export function Timeline({ revision, tags, filters, setFilters, onEdit, initialPage, visible = true, dateLocked = false }: {
    revision: number;
    ...
    const generation = useRef(0), inflight = useRef(false), sentinel = useRef<HTMLDivElement>(null), dateInput = useRef<HTMLInputElement>(null);
    ...
    const active = !!(filters.q || filters.tag || (!dateLocked && filters.date));
    const groups = [...new Set(entries.map(e => e.date))];
    const chooseDate = (value: string) => { if (value !== filters.date) setFilters({ ...filters, date: value }); };
    return <section className="timeline-section" aria-label="その時の記録の履歴"><div className="section-heading"><div><p className="eyebrow">TIMELINE</p><h2>記録をたどる</h2></div><div className="timeline-actions">{!dateLocked && <><button type="button" className={`icon-button history-date-trigger ${filters.date ? 'selected' : ''}`} aria-label={filters.date ? `${filters.date}で絞り込み中` : '日付で絞り込む'} title="日付で絞り込む" onClick={() => { const input = dateInput.current; if (!input) return; const picker = (input as HTMLInputElement & { showPicker?: () => void }).showPicker; if (picker) picker.call(input); else input.click(); }}><CalendarDays size={19}/></button><input ref={dateInput} className="history-date-input" type="date" aria-label="履歴の日付" max={todayKey()} value={filters.date} onInput={e => chooseDate(e.currentTarget.value)} onChange={e => chooseDate(e.target.value)}/></>}<button type="button" className={`secondary-button ${open || active ? 'selected' : ''}`} aria-expanded={open} onClick={() => setOpen(!open)}><SlidersHorizontal size={16}/>絞り込み{active && <span className="filter-count">ON</span>}</button></div></div>
 {(open || filters.q || filters.tag) && <div className="history-tools"><label className="search-field"><Search size={18}/><input aria-label="言葉で検索" placeholder="メモやタグを検索" value={filters.q} onChange={e => setFilters({ ...filters, q: e.target.value })}/></label><Select value={filters.tag || '__all'} onValueChange={v => setFilters({ ...filters, tag: v === '__all' ? '' : v })}><SelectTrigger aria-label="タグで絞り込む"><SelectValue placeholder="すべてのタグ"/></SelectTrigger><SelectContent><SelectItem value="__all">すべてのタグ</SelectItem>{tags.map(t => <SelectItem key={t} value={t}>#{t}</SelectItem>)}</SelectContent></Select><div className="history-date-shortcuts">{!dateLocked && <><button className={`text-button ${filters.date === todayKey() ? 'selected' : ''}`} onClick={() => chooseDate(todayKey())}>今日</button><button className={`text-button ${!filters.date ? 'selected' : ''}`} onClick={() => chooseDate('')}>全期間</button></>}{active && <button className="text-button" onClick={() => setFilters({ q: '', tag: '', date: dateLocked ? filters.date : '' })}><X size={14}/>解除</button>}</div></div>}
