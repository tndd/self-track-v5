...
import { Frown, Annoyed, Meh, Smile, SmilePlus, Plus, Minus, X, Tag, Check, Folder, Clock, Sparkles } from 'lucide-react';
...
export function TagPicker({ all, value, onChange, onAdd, catalog, usage = {}, recent = [], expanded = true, quantities = {}, onQuantities }: {
    ...
    return <div className="tag-picker">
      {quick.length > 0 && <><div className="tag-quick-label"><Clock size={13}/>最近使ったタグ</div>{choiceRow(quick, '最近のタグ')}</>}
      <div className="tag-quick-label"><Sparkles size={13}/>{frequent.length ? FREQUENT : (catalog ? 'タグを選ぶ' : 'タグ')}</div>
